...
| Google Fonts Knowledge — Foundations of Web Typography | `fonts.google.com/knowledge/using_type/the_foundations_of_web_typography` | **Unconfirmed** — likely CC BY 4.0 but page is JavaScript-rendered and could not be verified by fetch | Pending manual license verification. Topics include optical sizing, variable fonts, tracking, responsive typography — all novel relative to current KB. **Plan:** Verify CC BY 4.0 in browser, then synthesize into typography foundations. Candidate for a new `responsive-typography.md` document. |
