...
### Logo SVG component
The Sistema logo SVG is in `src/components/Logo.tsx`. It exports two components: `Logo` (size prop, renders the SVG symbol — yellow circle, blue rotated square/rhombus, red triangle with pre-mixed purple and green overlap regions) and `Wordmark` (renders Logo + "Sistema" in Fraunces serif). The SVG paths are hardcoded from the original `sistema-logo.svg` upload (172×192 native viewBox). Aspect ratio is always 172/192; height is computed from size as `Math.round(size * (192/172))`. Use `<Wordmark />` in the nav; use `<Logo size={N} />` standalone in footers or illustrations.

### PromptBox component
`src/components/PromptBox.tsx` is the standard LLM-style prompt surface used across the app (home hero, play detail, campaign step). Props: `label` (mono header text), `body` (copyable string), `tokens?`, `refs?`, `variables?` (stat footer), `expanded?` (default false — shows 4–5 lines with gradient fade and "Show full prompt" pill). Copy button is in the header (blue, with check icon on success). Footer shows "paste into Claude Code / Cursor" labels. The `{{sistema_url}}` placeholder in body is resolved at copy time using `window.location.origin`. For campaign steps with variable substitution, use the inline prompt block pattern in `CampaignStep.tsx` directly rather than PromptBox (variable resolution happens in the component's closure).

### Page layout convention (Phase 10+)
All major pages use `max-w-[1180px] mx-auto px-10` as their content container — wider than the previous `max-w-5xl px-6` convention. The body background is canvas (#EFEEEA); individual page content areas use `bg-surface` (white) only when needed for card/raised surfaces — the canvas shows through in padding/margin areas. Full-bleed colored sections (e.g. campaign step blue header) should break out of the container using a wrapper div at `<main>` level, then re-contain with the standard max-width inside.

### Stage chip color system
Six stage chips use distinct color palettes (not just Tailwind grays): stage 1 `bg #E6F0FF / text #0058CC / border #BFD8FF`; stage 2 `bg #E6F0FF / text #1056BF / border #BFD8FF`; stage 3 `bg #F0E9FF / text #5325B8 / border #D9C9FF`; stage 4 `bg #E3F3EC / text #137A4D / border #BCE3CE`; stage 5 `bg #FFF1E0 / text #A65B00 / border #FFD9A8`; stage 6 `bg #FFF8E0 / text #8A6500 / border #F2DA8A`. These are consistent across home QuickCard row, plays index StageRow, and play detail breadcrumb chips. Defined as a static STAGE_CHIP_STYLES record in each page/component that uses them — not yet a shared constant.
